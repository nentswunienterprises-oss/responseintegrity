Warning: truncated output (original token count: 308444)
... 185198 bytes omitted ...
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          res.json({
            ...
            parent_confirmed: resolvedSession.parent_co…62152 tokens truncated…(endpoint);
      res.status(204).send();
    } catch (error) {
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const durableUser = await storage.getUser(userId);
        const durableUserProductionCode = normalizeProductionLinkCode((durableUser as any)?.productionLinkCode);
        const durableUserTrackingSource = (durableUser as any)?.trackingSource || null;
        const durableUserTrackingCampaign = (durableUser as any)?.trackingCampaign || null;
        ...
        try {
          durableProductionCode = resolveDurableProductionLink(
            durableUserProductionCode,
            existingProductionCode,
            incomingProductionCode,
          );
        } catch (error) {
        ...
        if (durableProductionCode) {
          ...
          const validationError = validateProductionLinkForRole(
            durableUserProductionCode ? { ...productionLink, status: "active" } : productionLink,
            "capacity",
            "tutor",
          );
          if (validationError) {
          ...
          } else if (productionLink.status !== "active") {
            return res.status(400).json({ message: "Production Link is invalid or inactive" });
          }
          ...
          if (!durableUserProductionCode) {
            await storage.claimUserProductionAttribution(
              userId,
              productionLink.production_link_code,
              (req.session as any)?.trackingSource || null,
              (req.session as any)?.trackingCampaign || null,
            );
          }
        }
        const data = insertTutorApplicationSchema.parse({
