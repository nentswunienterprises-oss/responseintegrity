...
CREATE TABLE IF NOT EXISTS public.capability_tps_timer_contract_invalidations (
  ...
  contract_id uuid NOT NULL REFERENCES public.capability_tps_timer_contracts(contract_id) ON DELETE RESTRICT,
  correction_id uuid NOT NULL REFERENCES public.response_integrity_evidence_corrections(correction_id) ON DELETE RESTRICT,
...
CREATE TRIGGER trg_capability_tps_timer_contract_invalidations_immutable
  ...
  FOR EACH ROW EXECUTE FUNCTION public.response_integrity_reject_evidence_mutation();
