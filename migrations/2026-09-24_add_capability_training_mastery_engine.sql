...
CREATE INDEX IF NOT EXISTS idx_capability_attempts_assessment
  ON specialist_capability_assessment_attempts (assessment_key, bank_version, completed_at DESC);

COMMENT ON TABLE private.specialist_capability_assessment_configs IS
  'Private Capability Engine assessment configuration. Answer-bearing content is not stored in the public repository.';

COMMENT ON TABLE private.specialist_capability_assessment_items IS
  'Private Capability Engine item bank including answer keys, explanations and critical-boundary metadata.';

COMMENT ON TABLE specialist_capability_assessment_attempts IS
  'Immutable submitted Specialist Training capability evidence with exact bank version and deterministic form identity.';
