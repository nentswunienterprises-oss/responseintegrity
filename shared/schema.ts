...
export const parentEnrollments = pgTable("parent_enrollments", {
  ...
  // Demand Production. Migration preserves pre-existing rows as version 0 with unknown evidence.
  demandFlowVersion: integer("demand_flow_version").notNull().default(1),
  qualificationStatus: varchar("qualification_status").default("pending"),
  qualificationOwnerId: varchar("qualification_owner_id").references(() => users.id),
  qualificationContactedAt: timestamp("qualification_contacted_at", { withTimezone: true }),
  qualificationCompletedAt: timestamp("qualification_completed_at", { withTimezone: true }),
  qualificationDecidedBy: varchar("qualification_decided_by").references(() => users.id),
  qualificationNote: text("qualification_note"),
  entrySelectedAt: timestamp("entry_selected_at", { withTimezone: true }),
  entrySelectedBy: varchar("entry_selected_by").references(() => users.id),
  handoverFromUserId: varchar("handover_from_user_id").references(() => users.id),
  handoverToUserId: varchar("handover_to_user_id").references(() => users.id),
