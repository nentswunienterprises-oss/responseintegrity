...
export default function IntroSessionDrillRunner() {
  ...
  const handleContinueAdaptiveTransition = () => {
    ...
    setAdaptiveDiagnosisMessage(
      `${adaptiveTransition.currentPhase} evidence resolved. ${
  ...
  const handleNext = async () => {
    ...
    if (shouldShowTopicReferenceCapture) {
      setTopicReferenceError("Save the Topic Reference before starting the observation drill sets.");
      return;
    }
    ...
    if (!isLastRep) {
      ...
      try {
        ...
        setAdaptiveDiagnosisMessage(
          isHandoverMode
            ? `Evidence in ${activeDiagnosisPhase} resolved targeted re-diagnosis to ${res.data?.summary?.resultingPhase || activeDiagnosisPhase}.`
            : `Evidence in ${activeDiagnosisPhase} locked Diagnosis at ${res.data?.summary?.phase || activeDiagnosisPhase}.`
        );
        setSubmitSuccess(true);
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 sm:p-6 space-y-4">
      ...
      {submitSuccess && scoring && scoring.length > 0 && (() => {
        // Group rows by topic so multi-topic sessions render evidence direction clearly
      ...
      {drillMode === "diagnosis" && showModeInstructions && (
        <div className="mb-4 rounded-md border border-primary/20 bg-primary/5 p-3">
          ...
          <ul className="list-disc pl-5 text-sm text-foreground/90 space-y-1">
            ...
            <li>The system selects the next evidence question or locks placement from the recorded behavior evidence.</li>
      ...
      {set?.isModelingSet && (
        <div className="mb-4 p-4 rounded-xl border border-primary/25 bg-primary/10">
          ...
          <div className="text-muted-foreground text-xs leading-relaxed">
            ...
            Sets 2 and 3 are the decision-eligible observation sets.
          </div>
        </div>
      )}
