...
try {
  ...
  assert.ok(
    signupResponse.ok(),
    `Signup failed: ${signupResponse.status()} ${JSON.stringify(signupBody)}`,
  );
  checkpoint("parent-signup-created", {
  ...
  await page.screenshot({
    path: path.join(evidenceDir, "02-signup-created.png"),
    fullPage: true,
  });
  ...
  for (let attempt = 1; attempt <= 36; attempt += 1) {
    ...
    lastSignin = {
      ...
      message: signinBody?.message || null,
    };

    try {
      await page.waitForURL(/\/client\/parent\/gateway/, { timeout: 7_000 });
      loggedIn = true;
      break;
    } catch {
      console.log(
        `LOGIN_WAIT attempt=${attempt} status=${signinResponse.status()} message=${signinBody?.message || "none"}`,
      );
  ...
  assert.ok(
    loggedIn,
    `Parent login did not reach Gateway after confirmation window. Last signin: ${JSON.stringify(lastSignin)}`,
  );
  checkpoint("parent-login-reached-gateway", {
    url: page.url(),
    lastSignin,
  });
  ...
  await page.screenshot({
    path: path.join(evidenceDir, "03-gateway-application.png"),
    fullPage: true,
  });
  ...
  await page
    .getByRole("button", { name: "No - this will be their first time", exact: true })
    .click();
  ...
  const processSection = processPrompt.locator("..");
  const processYes = processSection.getByRole("button", { name: "Yes", exact: true });
  if (await processYes.count()) {
    await processYes.click();
  }
  ...
  const enrollmentResponsePromise = page.waitForResponse(
    (response) =>
      response.url().includes("/api/parent/enroll") &&
      response.request().method() === "POST",
    { timeout: 60_000 },
  );
  ...
  assert.ok(
    ...
    `Enrollment submission failed: ${enrollmentResponse.status()} ${JSON.stringify(enrollmentBody)}`,
  );

  await page
    .getByText("Application received. We're assessing fit.", { exact: true })
  ...
  await page.screenshot({
    path: path.join(evidenceDir, "04-application-pending-review.png"),
    fullPage: true,
  });
